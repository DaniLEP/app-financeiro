...
const ListaChamada = () => {
  ...
  const confirmarPresenca = (idEvento, idConvite, tipo) => {
    const statusField = tipo === "Comprador" ? "statusComprador" : "statusConvidado";
    const conviteRef = ref(dbRealtime, `convites/${idEvento}/${idConvite}`);

    update(conviteRef, { [statusField]: "presente" })
      .then(() => console.log(`${tipo} marcado como presente!`))
  ...
  return (
    <div className="p-4 md:p-6">
      <h1 className="text-2xl font-bold mb-4">📋 Lista de Chamada</h1>
          ...
          <table className="min-w-full text-sm text-left border-collapse">
            <thead className="bg-gray-100 text-gray-700">
              <tr>
                ...
                <th className="px-4 py-2 border">Presença</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="7" className="text-center py-4">Nenhum convite encontrado</td>
                </tr>
              ) : (
                convitesFiltrados.map((c) => (
                  <>
                    ...
                    <tr key={c.id + "-comprador"} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 border">{c.comprador?.cpf}</td>
                      <td className="px-4 py-2 border">
                        {c.statusComprador === "presente" ? (
                          "✅ Presente"
                        ) : (
                          <button
                            onClick={() => confirmarPresenca(c.idEvento, c.id, "Comprador")}
                    ...
                    {c.convidado?.nome && (
                      <tr key={c.id + "-convidado"} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 border">{c.convidado?.cpf}</td>
                        <td className="px-4 py-2 border">
                          {c.statusConvidado === "presente" ? (
                            "✅ Presente"
                          ) : (
                            <button
                              onClick={() => confirmarPresenca(c.idEvento, c.id, "Convidado")}
                              className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600"
                            >
                              Confirmar
                            </button>
      ...
      {/* Para telas pequenas: cards empilhados */}
      <div className="md:hidden space-y-4">
        {loading ? (
          <p className="text-center py-4">Carregando...</p>
        ) : convitesFiltrados.length === 0 ? (
          <p className="text-center py-4">Nenhum convite encontrado</p>
        ) : (
          convitesFiltrados.map((c) => (
            <div key={c.id + "-card"} className="border rounded-lg p-4 shadow-sm bg-white">
              ...
              {c.convidado?.nome && (
                <>
                  ...
                  <p><strong>Tipo:</strong> Convidado</p>
                  <p><strong>Nome:</strong> {c.convidado?.nome} {c.convidado?.sobrenome}</p>
                  <p><strong>Telefone:</strong> {c.convidado?.telefone}</p>
                  <p><strong>CPF:</strong> {c.convidado?.cpf}</p>
                  <p>
                    <strong>Presença:</strong>{" "}
                    {c.statusConvidado === "presente" ? (
                      "✅ Presente"
